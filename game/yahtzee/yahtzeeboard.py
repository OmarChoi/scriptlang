...
class YahtzeeBoard:
    ...
    def diceListener(self, row):
        if self.diceButtons[row]['text'] != '':    # 맨 처음 돌릴 때는 못잠그게 하기
    ...
    def categoryListener(self, row):
        ...
        if cur_player.getAtUsed(index) == True or self.diceButtons[0]['text'] == '':
           return
        cur_player.setScore(score, index)
        cur_player.setAtUsed(index)
        self.fields[row][self.player]['text'] = str(score)
        ...
        for i in range(0, 6):
            if self.players[self.player].getAtUsed(i) == True:
                self.fields[i][self.player]['state'] = 'disabled'
                self.fields[i][self.player]['bg'] = 'light gray'

        for i in range(6, 13):
            if self.players[self.player].getAtUsed(i) == True:
                self.fields[i + 2][self.player]['state'] = 'disabled'
                self.fields[i + 2][self.player]['bg'] = 'light gray'
